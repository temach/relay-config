...
namespace JsonToRelay
{
  ...
  class MainLogic : IDisposable
  {
    ...
    static public MainLogic MakeMainLogic()
    {
      var ret = new MainLogic();
      // wait for device to be found
      while (ret.USB_Connect == false)
      {
        // in millisecs
        Thread.Sleep(500);
      }
      // read the time config, must be first action
      while (ret.WaitingForRead)
      {
        ret.usb_OnDataRecieved_WaitAndProcess();
      }
      return ret;
    }
    ...
    private void usb_OnDataReceivedAddToQueue(object sender, DataRecievedEventArgs args)
    {
      ...
      // UsbReceivedDataQueue.CompleteAdding();
    }
    ...
    public void OpenFileJSON(string path)
    {
      Tuple<DeviceOptions, CtrlProgramOptions> ret = jsonReadWrite.OpenFileJSON(path);
      this.DeviceOpts = ret.Item1;
      this.CtrlProgramsOpts = ret.Item2;
    }

    public void SaveFileJSON(string PathOfFile)
    {
      jsonReadWrite.SaveFileJSON(PathOfFile, this.DeviceOpts, this.CtrlProgramsOpts);
    }
