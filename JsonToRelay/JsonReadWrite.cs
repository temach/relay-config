...
namespace JsonToRelay
{
  class JsonReadWrite
  {
    ...
    public Tuple<DeviceOptions,CtrlProgramOptions> OpenFileJSON(string path)
    {
    ...
    public void SaveFileJSON(string PathOfFile, DeviceOptions devOpts, CtrlProgramOptions progsOpts)
    {
      this.DeviceOpts = devOpts;
      this.CtrlProgramsOpts = progsOpts;
      TimerClass[] timerClassArray = new TimerClass[6];
      using (FileStream fileStream = File.Open(PathOfFile, FileMode.Create, FileAccess.Write))
      {
